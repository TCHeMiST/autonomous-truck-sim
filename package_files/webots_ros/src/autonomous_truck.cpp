...
void quit(int sig) {
  ROS_INFO("User stopped the 'autonomous_truck' node.");
...
int main(int argc, char **argv) {
  ...
  if (argc != 1) {
    ROS_INFO("Usage: $ autonomous_truck.");
    return 1;
  }

  // create a node named 'autonomous_truck' on ROS network
  ros::init(argc, argv, "autonomous_truck", ros::init_options::AnonymousName);
  ...
  timeStepClient = n->serviceClient<webots_ros::set_int>("autonomous_truck/robot/time_step");
  ...
  ros::ServiceClient set_pc_client;
  webots_ros::set_int lidar_srv;
  webots_ros::set_bool lidar_bool;

  ros::Subscriber sub_lidar_scan;

  set_lidar_client = n->serviceClient<webots_ros::set_int>("autonomous_truck/Velodyne_VLP_16/enable");
  set_pc_client = n->serviceClient<webots_ros::set_bool>("autonomous_truck/Velodyne_VLP_16/enable_point_cloud");

  lidar_srv.request.value = TIME_STEP;
  lidar_bool.request.value = true;

  if (set_lidar_client.call(lidar_srv) && set_pc_client.call(lidar_bool) && lidar_srv.response.success) {
  ...
  // // enable Velodyne Puck lidar
  // ros::ServiceClient set_lidar_puck_client;
  // webots_ros::set_int lidar_puck_srv;
  // ros::Subscriber sub_lidar_puck_scan;

  // set_lidar_puck_client = n->serviceClient<webots_ros::set_int>("autonomous_truck/Velodyne_Puck/enable");
  // lidar_puck_srv.request.value = TIME_STEP;
  // if (set_lidar_puck_client.call(lidar_puck_srv) && lidar_puck_srv.response.success) {
  //   ROS_INFO("Puck enabled.");
  //   sub_lidar_puck_scan = n->subscribe("autonomous_truck/Velodyne_Puck/laser_scan/layer0", 10, lidarCallback);
  //   ROS_INFO("Topic for Puck initialized.");
  //   while (sub_lidar_puck_scan.getNumPublishers() == 0) {
  //   }
  //   ROS_INFO("Topic for Puck scan connected.");
  // } else {
  //   if (!lidar_puck_srv.response.success)
  //     ROS_ERROR("Sampling period is not valid.");
  //   ROS_ERROR("Failed to enable Puck.");
  //   return 1;
  // }
  ...
  set_GPS_client = n->serviceClient<webots_ros::set_int>("autonomous_truck/gps/enable");
  GPS_srv.request.value = 32;
  if (set_GPS_client.call(GPS_srv) && GPS_srv.response.success) {
    sub_GPS = n->subscribe("autonomous_truck/gps/values", 1, GPSCallback);
  ...
  set_inertial_unit_client = n->serviceClient<webots_ros::set_int>("autonomous_truck/inertial_unit/enable");
  inertial_unit_srv.request.value = 32;
  if (set_inertial_unit_client.call(inertial_unit_srv) && inertial_unit_srv.response.success) {
    sub_inertial_unit = n->subscribe("autonomous_truck/inertial_unit/roll_pitch_yaw", 1, inertialUnitCallback);
  ...
  set_accelerometer_client = n->serviceClient<webots_ros::set_int>("autonomous_truck/accelerometer/enable");
  ...
  set_camera_client = n->serviceClient<webots_ros::set_int>("autonomous_truck/camera/enable");
  ...
  set_gyro_client = n->serviceClient<webots_ros::set_int>("autonomous_truck/gyro/enable");
